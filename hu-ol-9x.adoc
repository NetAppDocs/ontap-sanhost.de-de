...
summary: Installieren Sie die Linux Host Utilities auf einem Oracle Linux 9.x Host, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 9.x für FCP und iSCSI mit ONTAP -Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Mit Oracle Linux 9.x können Sie Multipathing zur Verwaltung von ONTAP LUNs nutzen.
...
Bei Oracle Linux 9.3, 9.2, 9.1 und 9.0 mit ONTAP -Speicher besteht folgendes bekannte Problem:
...
| https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1508554["1508554"^] | FÜR DAS SAN-LUN-Dienstprogramm mit Emulex HBA werden symbolische Links von Bibliothekspaketen benötigt | Wenn Sie den Befehl „sanlun fcp show adapter -v“ der Linux Host Utilities CLI auf einem SAN-Host ausführen, schlägt der Befehl mit einer Fehlermeldung fehl, die besagt, dass die für die Erkennung eines Hostbusadapters (HBA) erforderlichen Bibliotheksabhängigkeiten nicht gefunden werden können: ---- [root@hostname ~]# sanlun fcp show adapter -v Die Bibliothek /usr/lib64/libHBAAPI.so konnte nicht gefunden werden. Stellen Sie sicher, dass das Paket, das die Bibliothek installiert, installiert und geladen ist. ---- 
|===
