...
summary: Installieren Sie Linux Host Utilities auf einem RHEL 9.x-Host, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie RHEL 9.x für FCP und iSCSI mit ONTAP -Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Mit RHEL 9.x können Sie Multipathing zur Verwaltung von ONTAP LUNs nutzen.
...
Bei RHEL 9.x mit ONTAP -Speicher gibt es folgende bekannte Probleme.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | Bei der NetApp Linux Host Utilities CLI sind zusätzliche Abhängigkeiten von Bibliothekspaketen erforderlich, um die Erkennung von Emulex HBA-Adaptern zu unterstützen | In RHEL 9.1 schlägt die NetApp Linux SAN Host Utilities-CLI `sanlun fcp show adapter -v` fehl, da die Abhängigkeiten des Bibliothekspakets zur Unterstützung der HBA-Erkennung nicht gefunden werden können. 
|===
--
====
