...
summary: Installieren Sie Linux Host Utilities auf einem Oracle Linux 8.9-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 8.9 für FCP und iSCSI mit ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Sie können Multipathing mit Oracle Linux 8.9 verwenden, um ONTAP-LUNs zu verwalten.


NOTE: Sie können die link:hu_rhel_89.html#rhel-rhck["empfohlene Einstellungen für Red Hat Enterprise Linux (RHEL) 8.9"] um den Red Hat Compatible Kernel für Oracle Linux 8.9 zu konfigurieren.
