...
summary: Beschreibt die Verwendung von Oracle Linux 8.9 mit ONTAP 
---
= Verwenden Sie Oracle Linux 8.9 mit ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP-SAN-Hosts verwenden, um Oracle Linux 8.9 mit ONTAP als Ziel zu konfigurieren.
...
.Was Sie benötigen
Ihre Konfiguration unterstützt das Booten von SAN. link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["NetApp Interoperabilitäts-Matrix-Tool"^]Überprüfen Sie im, ob Betriebssystem, HBA, HBA-Firmware und HBA-Boot-BIOS sowie ONTAP-Version unterstützt werden.
...
Für Oracle Linux (OL) 8.9 `/etc/multipath.conf` muss die Datei vorhanden sein. Sie müssen keine Änderungen an dieser Datei vornehmen, da Oracle Linux 8.9 mit allen Einstellungen kompiliert ist, die erforderlich sind, um ONTAP-LUNs zu erkennen und korrekt zu verwalten.
...
NOTE: Für eine einzelne LUN sollten nicht mehr als vier Pfade erforderlich sein. Mehr als acht Pfade können bei Storage-Ausfällen zu Pfadproblemen führen.
...
Das Oracle Linux 8.9-Betriebssystem wird kompiliert, um ONTAP-LUNs zu erkennen und automatisch die richtigen Konfigurationsparameter für ASA- und nicht-ASA-Konfigurationen festzulegen. Mithilfe der folgenden empfohlenen Einstellungen können Sie die Performance für Ihre Host-Konfiguration weiter optimieren.
...
NOTE: Um Oracle Linux 8.9 RedHat Enterprise Kernel link:hu_rhel_89.html#recommended-settings["Empfohlene Einstellungen"]zu konfigurieren, verwenden Sie den  für Red hat Enterprise Linux (RHEL) 8.9.



== KVM-Einstellungen

Sie können auch die empfohlenen Einstellungen verwenden, um eine Kernel-basierte virtuelle Maschine (KVM) zu konfigurieren. Zum Konfigurieren einer KVM sind keine Änderungen erforderlich, da die LUN dem Hypervisor zugeordnet ist.



== Bekannte Probleme

Es gibt keine bekannten Probleme für die Oracle Linux 8.9 mit ONTAP-Version.
