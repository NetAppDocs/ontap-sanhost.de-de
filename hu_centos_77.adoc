...
summary: Beschreibt die Verwendung von CentOS 7.7 mit ONTAP 
---
= Verwenden Sie CentOS 7.7 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP-SAN-Hosts verwenden, um CentOS 7.7 mit ONTAP als Ziel zu konfigurieren.
...
Für CentOS 7.7 muss die Datei /etc/Multipath.conf vorhanden sein, aber Sie müssen keine spezifischen Änderungen an der Datei vornehmen. CentOS 7.7 wird mit allen Einstellungen zusammengestellt, die zum Erkennen und korrekten Managen von ONTAP LUNs erforderlich sind.
...
Das CentOS 7.7-Betriebssystem wird kompiliert, um ONTAP-LUNs zu erkennen und automatisch alle Konfigurationsparameter für die ASA- und nicht-ASA-Konfiguration korrekt einzustellen. Die `multipath.conf` Datei muss vorhanden sein, damit der Multipath-Daemon gestartet werden kann. Wenn diese Datei nicht vorhanden ist, können Sie eine leere Datei mit einem Zero-Byte-Wert erstellen, indem Sie den folgenden Befehl verwenden:
...
CentOS 7.7 mit ONTAP-Version weist folgende bekannte Probleme auf:
