...
summary: So konfigurieren Sie einen NVMe-oF-Host für Rocky Linux 9.6 mit ONTAP 
---
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Rocky Linux 9.6 konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP Unterstützung und Funktionen"]Die

*NVMe-oF mit Rocky Linux 9.6 hat die folgenden bekannten Einschränkungen:*
...
.Schritte
. Installieren Sie Rocky Linux 9.6 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen Rocky Linux 9.6-Kernel ausführen:
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Generieren Sie den DHCP-Schlüssel für den Rocky Linux 9.6-Host.
+
