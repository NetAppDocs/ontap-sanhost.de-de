...
summary: Installieren Sie Linux Host Utilities auf einem CentOS 8.1-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie CentOS 8.1 für FCP und iSCSI für ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software enthält Management- und Diagnose-Tools für Linux-Hosts, die mit dem ONTAP-Storage verbunden sind. Wenn Sie die Linux Host Utilities auf einem CentOS 8.1-Host installieren, können Sie die Host Utilities zur Verwaltung von FCP- und iSCSI-Protokollvorgängen mit ONTAP LUNs verwenden.
...
Sie können Multipathing mit CentOS 8.1 verwenden, um ONTAP LUNs zu verwalten.
...
Es sind keine Probleme für CentOS 8.1 für FCP und iSCSI mit ONTAP-Speicher bekannt.

Informationen zu bekannten Problemen mit dem CentOS Red Hat-kompatiblen Kernel finden Sie im link:hu_rhel_81.html#known-problems-and-limitations["Bekannte Probleme"] für Red Hat Enterprise Linux (RHEL) 8.1.
