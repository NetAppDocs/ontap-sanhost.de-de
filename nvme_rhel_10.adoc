...
summary: Konfigurieren des NVMe-of-Hosts für RHEL 10 mit ONTAP 
---
= NVMe-of Hostkonfiguration für RHEL 10 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.Schritte
. Installieren Sie RHEL 10 auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen RHEL 10-Kernel ausführen:
+
[source, cli]
----
uname -r
...
Mit RHEL 10 wird die  `nvmefc-boot-connections.service` Und  `nvmf-autoconnect.service` In NVMe/FC enthaltene Boot-Dienste  `nvme-cli` Pakete werden beim Systemstart automatisch aktiviert. Überprüfen Sie nach dem Booten, ob die Dienste aktiviert sind.
...
cat /etc/nvme/hostnqn
----
. Generieren Sie den Dhchap-Schlüssel für den RHEL 10-Host.
+
