...
summary: Installieren Sie Linux Host Utilities auf einem Oracle Linux 7.1-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 7.1 für FCP und iSCSI mit ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Sie können Multipathing mit Oracle Linux 7.1 verwenden, um ONTAP-LUNs zu verwalten. Oracle Linux 7.1 unterstützt Unbreakable Enterprise Kernel (UEK) R3 und UEK R4. Das OS startet standardmäßig mit UEK R3 Kernel.


NOTE: Sie können die link:hu_rhel_71.html#rhel-rhck["empfohlene Einstellungen für Red Hat Enterprise Linux (RHEL) 7.1"] um den Red Hat Compatible Kernel für Oracle Linux 7.1 zu konfigurieren.
