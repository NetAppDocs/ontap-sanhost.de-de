...
Um sicherzustellen, dass die Solaris Client-Anwendungen bei einer ungeplanten Failover-Umschaltung in einer SnapMirror Business Continuity (SM-BC)-Umgebung unterbrechungsfrei laufen, muss auf dem Solaris 11.3 Host die folgende Einstellung konfiguriert werden. Diese Einstellung überschreibt das Failover-Modul – `f_tpgs` Um zu verhindern, dass der Codepfad, der den Widerspruch erkennt, ausgeführt wird.


NOTE: Ab ONTAP 9.9 werden SM-BC-Einstellungskonfigurationen auf Solaris 11.3 Host unterstützt.
...
"NETAPP  LUN","f_tpgs"
----
. Verwenden Sie die `devprop` Und `mdb` Befehle, mit denen überprüft werden soll, ob der Override-Parameter erfolgreich angewendet wurde:
+
...
NOTE: `conf` Wird hinzugefügt zu `svl_fops_name` Wenn `scsi-vhci-failover-override` Wurde angewendet. Weitere Informationen und empfohlene Änderungen an den Standardeinstellungen finden Sie im NetApp KB-Artikel https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration["Solaris Host Support Empfohlene Einstellungen in SnapMirror Business Continuity (SM-BC)-Konfiguration"^].
