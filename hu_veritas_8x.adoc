...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP SAN-Hosts für die Version 8 der Veritas Infoscale Storage Foundation für Red hat Enterprise Linux- und Oracle Linux-Plattformen (basierend auf Red hat Compatible Kernel) mit FC-, FCoE- und iSCSI-Protokollen verwenden.
