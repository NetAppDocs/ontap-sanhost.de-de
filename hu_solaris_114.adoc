...
Um sicherzustellen, dass die Solaris Client-Anwendungen bei einer ungeplanten Failover-Umschaltung in einer SnapMirror Business Continuity (SM-BC)-Umgebung unterbrechungsfrei laufen, muss auf dem Solaris 11.4 Host die folgende Einstellung konfiguriert werden. Diese Einstellung überschreibt das Failover-Modul – `f_tpgs` Um zu verhindern, dass der Codepfad, der den Widerspruch erkennt, ausgeführt wird.
...
"NETAPP  LUN","f_tpgs"
----
. Nutzung `devprop` Und `mdb` Befehle, mit denen überprüft werden soll, ob der Override-Parameter erfolgreich angewendet wurde:
+
