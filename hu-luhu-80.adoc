...
.Über diese Aufgabe
Bei der Installation der Linux Host Utilities ändert sich keine Einstellung und kein Verhalten auf Ihrem Host.
...
NOTE: Die Linux Host Utilities haben eine Installationsabhängigkeit von der Bibliothek libHBAAPI.so. Eines der oben aufgeführten Pakete muss installiert werden, selbst wenn ein Host ausschließlich für eine iSCSI-only-Konfiguration vorgesehen ist. Alternativ können OS-Tools für die LUN- und Pfadverwaltung verwendet werden.
