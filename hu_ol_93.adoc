...
summary: Installieren Sie Linux Host Utilities auf einem Oracle Linux 9.3-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 9.3 für FCP und iSCSI mit ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Sie können Multipathing mit Oracle Linux 9.3 verwenden, um ONTAP-LUNs zu verwalten.


NOTE: Sie können die link:hu_rhel_93.html#rhel-rhck["empfohlene Einstellungen für Red Hat Enterprise Linux (RHEL) 9.3"] um den Red Hat Compatible Kernel für Oracle Linux 9.3 zu konfigurieren.
...
Bei der Version Oracle Linux 9.3 mit ONTAP-Speicher tritt das folgende bekannte Problem auf:
