...
. Installieren Sie RHEL 8.5 GA auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen RHEL 8.5 GA-Kernel ausführen. Siehe link:https://mysupport.netapp.com/matrix/["NetApp Interoperabilitätsmatrix"^] Erhalten Sie auf der aktuellen Liste der unterstützten Versionen.
+
Beispiel:
...
Um bequem über ONTAP-Failover-Ereignisse wie SFOs zu tide, wird empfohlen, eine längere Wiederholungsdauer wie etwa 30 Minuten zu setzen, indem Sie den voreingestellt haben `ctrl_loss_tmo` Timer. Im Folgenden sind die Details aufgeführt:
...
----
. Führen Sie die aus `nvme connect-all` Befehl über alle unterstützten NVMe/TCP-Initiator-Ziel-LIFs über die Nodes hinweg Stellen Sie sicher, dass Sie länger bestehen `ctrl_loss_tmo` Zeitraum (z. B. 30 Minuten, die durch eingestellt werden können `-l 1800`) Während des connect-all so, dass es für einen längeren Zeitraum im Falle eines Pfadverlusts wiederholt. Beispiel:
+
[listing]
----
