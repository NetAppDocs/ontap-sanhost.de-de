...
---
= Konfigurieren Sie RHEL 8.8 für NVMe-oF mit ONTAP Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für RHEL 8.8 konfigurieren.  Weitere Informationen zu Support und Funktionen finden Sie unterlink:hu-nvme-index.html["NVME-oF-Übersicht"^] .

*NVMe-oF mit RHEL 8.8 weist die folgenden bekannten Einschränkungen auf:*

* SAN-Booten mit dem NVMe-oF-Protokoll wird derzeit nicht unterstützt.
* In-Kernel-NVMe-Multipath ist auf NVMe-oF-Hosts in RHEL 8.8 standardmäßig deaktiviert; Sie müssen es manuell aktivieren.
...
.Schritte
. Installieren Sie RHEL 8.8 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen RHEL 8.8-Kernel ausführen:
+
[source, cli]
----
uname -r
...
----
. Überprüfen Sie auf dem RHEL 8.8-Host die `hostnqn` Zeichenfolge bei `/etc/nvme/hostnqn` :
+
[source, cli]
----
