...
[role="lead"]
Ungelöste Direktive in <stdin> – include::_include/nvme/reuse-nvme-introduction.adoc[]
...
Ungelöste Direktive in <stdin> – include::_include/nvme/reuse_nvme_enabling_broadcom_1mb_size.adoc[]
...
Ungelöste Direktive in <stdin> – include::_include/nvme/reuse-nvme-configure-nvme-tcp.adoc[]
...
Ungelöste Direktive in <stdin> – include::_include/nvme/reuse-nvme-validate-nvme-of.adoc[]

.Schritte
. Überprüfen Sie die folgenden NVMe/FC-Einstellungen auf dem Oracle Liniux 9.5-Host:
+
.. Überprüfen Sie, ob „Multipath“ eingestellt ist:
+
[source, cli]
----
...
----
+
Der Wert  `Y` überprüft, ob der Multipath-Parameter festgelegt ist.

.. Anzeige der Subsysteme:
+
[source, cli]
----
