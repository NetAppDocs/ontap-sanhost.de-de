...
NOTE: Um Oracle Linux 8.10 RedHat Enterprise Kernel link:hu_rhel_810.html#recommended-settings["Empfohlene Einstellungen"]zu konfigurieren, verwenden Sie den  für Red hat Enterprise Linux (RHEL) 8.10.



== KVM-Einstellungen

Sie können das auch verwenden<<recommended_settings_ol_810,Empfohlene Einstellungen>>, um eine Kernel-basierte virtuelle Maschine (KVM) zu konfigurieren. Zum Konfigurieren einer KVM sind keine Änderungen erforderlich, da die LUN dem Hypervisor zugeordnet ist.
