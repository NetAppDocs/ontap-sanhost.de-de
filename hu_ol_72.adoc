...
summary: Installieren Sie Linux Host Utilities auf einem Oracle Linux 7.2-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 7.2 für FCP und iSCSI mit ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Sie können Multipathing mit Oracle Linux 7.2 verwenden, um ONTAP-LUNs zu verwalten. Oracle Linux 7.2 unterstützt Unbreakable Enterprise Kernel (UEK) R3 und UEK R4. Das Betriebssystem startet standardmäßig mit dem UEK R3-Kernel


NOTE: Sie können die link:hu_rhel_72.html#rhel-rhck["empfohlene Einstellungen für Red Hat Enterprise Linux (RHEL) 7.2"] um den Red Hat Compatible Kernel für Oracle Linux 7.2 zu konfigurieren.
