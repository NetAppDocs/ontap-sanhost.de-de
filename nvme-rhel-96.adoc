...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Red Hat Enterprise Linux (RHEL) 9.6 konfigurieren.  Weitere Informationen zu Support und Funktionen finden Sie unterlink:hu-nvme-index.html["NVME-oF-Übersicht"^] .

*NVMe-oF mit RHEL 9.6 weist die folgenden bekannten Einschränkungen auf:*
...
.Schritte
. Installieren Sie RHEL 9.6 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen RHEL 9.6-Kernel ausführen:
+
[source, cli]
----
uname -r
...
=== Ändern Sie die iopolicy in den udev-Regeln

RHEL 9.6 setzt die Standard-IO-Richtlinie für NVMe-oF auf `round-robin` .  Wenn Sie die iopolicy ändern möchten in `queue-depth` , ändern Sie die udev-Regeldatei wie folgt:
...
----
. Führen Sie die folgenden Befehle aus, um die Udev-Regeln neu zu laden und die Änderungen anzuwenden:
+
[source, cli]
----
...
----
. Überprüfen Sie die aktuelle IO-Richtlinie für Ihr Subsystem.  Ersetzen Sie <Subsystem> durch den entsprechenden Namen, zum Beispiel: `nvme-subsys0` .
+
[source, cli]
----
