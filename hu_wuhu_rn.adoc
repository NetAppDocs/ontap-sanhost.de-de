...
| MCS wird beim Booten der iSCSI-Software nicht unterstützt | Wenn die iSCSI-Software mit iSCSI Multiple Connections per Session (MCS) gestartet wird, kann die führende iSCSI-Boot-Verbindung nicht geschützt werden. Dies kann zur Trennung der Boot-LUN führen, was zu einer Unterbrechung des Host-Systems führt. | Microsoft unterstützt MCS für über iSCSI-Software gestartete Hosts nicht. Setzen Sie MPIO als Hochverfügbarkeitslösung ein. Beachten Sie, dass die Kombination von MPIO und MCS nicht unterstützt wird. 
|===
