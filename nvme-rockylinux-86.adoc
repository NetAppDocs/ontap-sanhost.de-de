...
summary: So konfigurieren Sie einen NVMe-oF-Host für Rocky Linux 8.6 mit ONTAP 
---
= Konfigurieren Sie Rocky Linux 8.6 mit NVMe-oF für ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.6-Hosts unterstützen die Protokolle NVMe/FC und NVMe/TCP mit Asymmetric Namespace Access (ANA). ANA entspricht dem Asymmetric Logical Unit Access (ALUA) Multipathing in iSCSI- und FCP-Umgebungen und wird mithilfe der In-Kernel-NVMe-Multipath-Funktion implementiert.
...
.Über diese Aufgabe
Sie können die folgende Unterstützung und Funktionen mit der NVMe-oF-Hostkonfiguration für Rocky Linux 8.6 verwenden. Sie sollten auch die bekannten Einschränkungen überprüfen, bevor Sie mit dem Konfigurationsprozess beginnen.
...
+
** In-Kernel-NVMe-Multipath ist für Rocky Linux 8.6 NVMe-oF-Hosts standardmäßig deaktiviert. Deshalb müssen Sie sie manuell aktivieren.
** Auf Rocky Linux 8.6-Hosts ist NVMe/TCP aufgrund offener Probleme eine Technologievorschaufunktion. Weitere Informationen finden Sie im https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.6_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.6 Versionshinweise"^] .
...
Verwenden Sie das folgende Verfahren, um die mindestens unterstützten Softwareversionen von Rocky Linux 8.6 zu validieren.

.Schritte
. Installieren Sie Rocky Linux 8.6 auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen Rocky Linux 8.6-Kernel verwenden:
+
[source, cli]
----
uname -r
...
Die NVMe-oF-Hostkonfiguration für Rocky Linux 8.6 mit ONTAP weist die folgenden bekannten Probleme auf:
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 8.6 NVMe-oF-Hosts erstellen doppelte Persistent Discovery Controller | Auf NVMe over Fabrics-Hosts (NVMe-of) können Sie den Befehl „nvme discover -p“ verwenden, um persistente Discovery Controller (PDCs) zu erstellen. Wenn dieser Befehl verwendet wird, sollte pro Initiator-Zielkombination nur ein PDC erstellt werden. Wenn Sie jedoch ONTAP 9.10.1 und Rocky Linux 8.6 mit einem NVMe-oF-Host ausführen, wird bei jeder Ausführung von „nvme discover -p“ ein doppelter PDC erstellt. Dies führt zu einer unnötigen Nutzung der Ressourcen auf dem Host und dem Ziel. 
|===