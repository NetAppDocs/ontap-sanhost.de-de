...
keywords: host utilities, oracle, linux, 10, netapp, ontap 
summary: Installieren Sie die Linux Host Utilities auf einem Oracle Linux 10 Host, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= Oracle Linux 10 für FCP und iSCSI mit ONTAP-Speicher konfigurieren
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software bietet Verwaltungs- und Diagnosewerkzeuge für Linux-Hosts, die mit ONTAP Storage verbunden sind. Wenn Sie die Linux Host Utilities auf einem Oracle Linux 10 Host installieren, können Sie die Host Utilities verwenden, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten.
...
Sie können Multipathing mit Oracle Linux 10 verwenden, um ONTAP LUNs zu verwalten.
