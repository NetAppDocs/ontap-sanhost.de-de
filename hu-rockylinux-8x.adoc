...
summary: Installieren Sie Linux Host Utilities auf einem Rocky Linux 8.10-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Rocky Linux 8.x für FCP und iSCSI mit ONTAP -Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software bietet Verwaltungs- und Diagnosetools für Linux-Hosts, die mit ONTAP Speicher verbunden sind. Wenn Sie Linux Host Utilities auf einem Rocky Linux 8.x-Host installieren, können Sie die Host Utilities verwenden, um FCP- und iSCSI-Protokolloperationen mit ONTAP -LUNs zu verwalten.
...
Mit Rocky Linux 8.x können Sie Multipathing zur Verwaltung von ONTAP LUNs nutzen.
