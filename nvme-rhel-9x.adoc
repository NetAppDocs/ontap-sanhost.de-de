...
summary: Konfiguration eines NVMe-oF-Hosts für RHEL 9x mit ONTAP 
---
= RHEL 9x für NVMe-oF mit ONTAP -Speicher konfigurieren
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für RHEL 9x konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rhel-supported-features.html["RHEL ONTAP Unterstützung und -Funktionen"]Die

NVMe-oF unter RHEL 9x weist folgende bekannte Einschränkungen auf:
...
.Schritte
. Installieren Sie RHEL 9x auf dem Server.  Nach Abschluss der Installation überprüfen Sie, ob Sie den erforderlichen RHEL 9x-Kernel ausführen:
+
[source, cli]
----
uname -r
...
Eine sichere In-Band-Authentifizierung wird über NVMe/TCP zwischen einem RHEL 9x-Host und einem ONTAP Controller unterstützt.
...
cat /etc/nvme/hostnqn
----
. Generieren Sie den DHCP-Schlüssel für den RHEL 9x-Host.
+
