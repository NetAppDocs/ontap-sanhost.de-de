...
.Änderungen an iopolicy für RHEL 9.x Releases
* Ab RHEL 9.7 ist die Standard-iopolicy queue-depth.
* In RHEL 9.6 ist round-robin die Standard-iopolicy, aber queue-depth ist als konfigurierbare Option verfügbar.
* Vor RHEL 9.6 war Round-Robin die einzige unterstützte NVMe-oF-iopolicy.


Wenn Sie RHEL 9.6 verwenden und die iopolicy auf  `queue-depth` ändern möchten, ändern Sie die udev-Regeldatei wie folgt:
