...
summary: Beschreibt die Verwendung von Red hat Enterprise Linux 8.7 mit ONTAP 
---
= Einsatz von Red hat Enterprise Linux 8.7 mit NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Für Red hat Enterprise Linux (RHEL) 8.7 die `/etc/multipath.conf` Die Datei muss vorhanden sein, Sie müssen jedoch keine spezifischen Änderungen an der Datei vornehmen. RHEL 8.7 wird mit allen Einstellungen zusammengestellt, die zum Erkennen und korrekten Managen von ONTAP-LUNs erforderlich sind.
...
Das RHEL 8.7 Betriebssystem wird kompiliert, um ONTAP-LUNs zu erkennen und automatisch alle Konfigurationsparameter für ASA- und nicht-ASA-Konfigurationen korrekt festzulegen.
...
| 1501048 | Zeitüberschreitungsparameter für die FC-Transportschicht `dev_loss_tmo` Für RemotedSCSI-Zielports wird ein negativer Wert angegeben (-1) |  `dev_loss_tmo` Der Parameter Timeout stellt die Standardanzahl von Sekunden dar, die der FC-Transport isolieren soll, um den Verlust eines Remote-Ports zu verhindern. Wenn dieser Wert überschritten wird, wird das scsi-Ziel vom Host entfernt. Dieser Parameterwert sollte ein positiver Ganzzahlwert sein. In RHEL 8.7 jedoch `sysfs` Dateisystem gibt einen negativen Wert (-1) für das zurück `dev_loss_tmo` Zeitüberschreitungsparameter für SCSI-Geräte. Nachfolgend das Beispiel der Befehlszeile zur Überprüfung des `dev_loss_tmo` Parameter auf einem arbeitenden Host:
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`. Es gibt keine Behelfslösung für dieses Problem in RHEL 8.7. | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
