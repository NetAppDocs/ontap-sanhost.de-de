...
summary: So konfigurieren Sie einen NVMe-oF-Host für Rocky Linux 9.1 mit ONTAP 
---
= Konfigurieren Sie Rocky Linux 9.1 mit NVMe-oF für ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 9.1-Hosts unterstützen die Protokolle NVMe/FC und NVMe/TCP mit Asymmetric Namespace Access (ANA). ANA entspricht dem Asymmetric Logical Unit Access (ALUA) Multipathing in iSCSI- und FCP-Umgebungen und wird mithilfe der In-Kernel-NVMe-Multipath-Funktion implementiert.
...
.Über diese Aufgabe
Sie können die folgende Unterstützung und Funktionen mit der NVMe-oF-Hostkonfiguration für Rocky Linux 9.1 verwenden. Sie sollten auch die bekannten Einschränkungen überprüfen, bevor Sie mit dem Konfigurationsprozess beginnen.
...
+
** Rocky Linux 9.1 aktiviert standardmäßig NVMe-Multipath im Kernel für NVMe-Namespaces, sodass keine expliziten Einstellungen mehr erforderlich sind.
...
Verwenden Sie das folgende Verfahren, um die mindestens unterstützten Rocky Linux 9.1-Softwareversionen zu validieren.

.Schritte
. Installieren Sie Rocky Linux 9.1 auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen Rocky Linux 9.1-Kernel verwenden:
+
[source, cli]
----
uname -r
...
Die NVMe-oF-Hostkonfiguration für Rocky Linux 9.1 mit ONTAP-Speicher weist das folgende bekannte Problem auf:
...
| 1503468 | `nvme list-subsys` Der Befehl gibt wiederholt aufgelistete NVMe-Controller für ein bestimmtes Subsystem zurück. | Der  `nvme list-subsys` Der Befehl sollte eine eindeutige Liste von NVMe-Controllern zurückgeben, die einem bestimmten Subsystem zugeordnet sind. In Rocky Linux 9.1 ist der  `nvme list-subsys` Der Befehl gibt NVMe-Controller mit ihrem jeweiligen ANA-Status für alle Namespaces zurück, die zu einem bestimmten Subsystem gehören. Der ANA-Status ist jedoch ein Namespace-Attribut. Daher sollten eindeutige NVMe-Controller-Einträge mit dem Pfadstatus angezeigt werden, wenn Sie die Subsystem-Befehlssyntax für einen bestimmten Namespace auflisten. 
|===