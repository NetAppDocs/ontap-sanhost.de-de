...
* Es gibt keine Unterstützung für NVMe-of für Sanlun. Daher gibt es keine Host Utility-Unterstützung für NVMe-of auf einem OL 8.8-Host. Sie können sich auf das im nativen `nvme-cli` Paket enthaltene NetApp Plug-in für alle NVMe-of-Übertragungen verlassen.
...
--
Der native Inbox qla2xxx Treiber, der im OL 8.8 GA Kernel enthalten ist, hat die neuesten Fixes. Diese Fehlerbehebungen sind für die Unterstützung von ONTAP unerlässlich.
...
| 1517321 | Oracle Linux 8.8 NVMe-of Hosts erstellen doppelte PDCs | Auf OL 8.8 NVMe-of-Hosts werden Persistent Discovery Controller (PDCs) erstellt, indem die `-p` Option an den `nvme discover` Befehl übergeben wird. Für eine bestimmte Kombination aus Initiator und Ziel `nvme discover` wird erwartet, dass bei jeder Ausführung des Befehls ein PDC erstellt wird. Ab OL 8.x erstellen NVMe-of-Hosts jedoch doppelte PDCs. Dadurch werden sowohl auf dem Host als auch auf dem Ziel Ressourcen verschwendet. 
|===