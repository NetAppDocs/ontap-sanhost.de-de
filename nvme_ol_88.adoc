...
summary: NVMe-of Host-Konfiguration für Oracle Linux 8.8 mit ONTAP 
---
= NVMe-of Host-Konfiguration für Oracle Linux 8.8 mit ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
NVMe over Fabrics (NVMe-of), einschließlich NVMe over Fibre Channel (NVMe/FC) und andere Übertragungen werden mit Oracle Linux (OL) 8.8 mit Asymmetric Namespace Access (ANA) unterstützt. In NVMe-of Umgebungen entspricht ANA ALUA Multipathing in iSCSI- und FC-Umgebungen und wird mit in-Kernel NVMe Multipath implementiert.

Folgende Unterstützung ist für die NVMe-of Hostkonfiguration für OL 8.8 mit ONTAP verfügbar:
...
* Es gibt keine Unterstützung für NVMe-of für Sanlun. Daher gibt es keine Host Utility-Unterstützung für NVMe-of auf einem OL 8.8-Host. Sie können sich auf das NetApp Plug-in verlassen, das im nativen Lieferumfang enthalten ist `nvme-cli` Paket für alle NVMe-of-Transporte.
...
* Oracle Linux 8.8 hat standardmäßig NVMe Multipath im Kernel für NVMe-Namespaces aktiviert, sodass keine expliziten Einstellungen erforderlich sind.
...
Sie können das folgende Verfahren verwenden, um die mindestens unterstützten OL 8.8-Softwareversionen zu validieren.

.Schritte
. Installieren Sie OL 8.8 GA auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen OL 8.8 GA-Kernel ausführen.
+
[listing]
----
# uname -r
...
----
. Überprüfen Sie auf dem Oracle Linux 8.8-Host die `hostnqn` Zeichenfolge bei `/etc/nvme/hostnqn`:
+
[listing]
----
...
--
.Schritte
. Der native Inbox qla2xxx Treiber im OL 8.8 GA Kernel enthält die neuesten Upstream-Fixes, die für die ONTAP Unterstützung unerlässlich sind. Vergewissern Sie sich, dass der unterstützte Adaptertreiber und die unterstützten Firmware-Versionen ausgeführt werden:
+
[listing]
----
...
Die NVMe-of-Hostkonfiguration für OL 8.8 mit ONTAP-Release hat die folgenden bekannten Probleme:
...
| 1517321 | Oracle Linux 8.8 NVMe-of Hosts erstellen doppelte Persistent Discovery Controller | Auf OL 8.8 NVMe-of-Hosts werden Persistent Discovery Controller (PDCs) durch Übergeben des erstellt `-p` Option für die `nvme discover` Befehl. Für eine bestimmte Kombination aus Initiator und Ziel wird erwartet, dass bei jedem Aufruf des nur ein PDC erstellt wird `nvme discover` Befehl. Ab OL 8.x erstellen NVMe-of-Hosts jedoch bei jedem Aufruf der doppelte PDCs `nvme discover` Befehl mit dem `-p` Option. Dadurch werden sowohl auf dem Host als auch auf dem Ziel Ressourcen verschwendet. | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118"^] 
|===