...
Wenn Ihr Host mit LUNs anderer Hersteller verbunden ist und einer der Multipath-Parameter überschrieben wurde, müssen diese korrigiert werden, indem Sie Abschnitte in der  `multipath.conf` Datei hinzufügen, die speziell für ONTAP LUNs gelten. Im unteren Bereich der Datei werden diese Abschnitte hinzugefügt. Werden die Parameter nicht korrigiert, funktionieren die ONTAP LUNs möglicherweise nicht wie erwartet.
