...
summary: Installieren Sie Linux Host Utilities auf einem HPE VME 8.0.x Host, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= HPE VME 8.0.x für FCP und iSCSI mit ONTAP-Speicher konfigurieren
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software bietet Verwaltungs- und Diagnosewerkzeuge für Linux-Hosts, die mit ONTAP-Speicher verbunden sind. Wenn Sie die Linux Host Utilities auf einem HPE VME 8.0.x-Host installieren, können Sie die Host Utilities verwenden, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten.



[[step-1-install-the-linux-host-utilities]]
== Schritt 1: Installieren Sie die Linux Host Utilities
...
Sie können Multipathing mit HPE VME 8.0.x verwenden, um ONTAP LUNs zu verwalten.
...
== Schritt 3: Bestätigen Sie die iSCSI-Konfiguration für Ihren Host
...
[listing]
----
iscsiadm --mode discovery --op update --type sendtargets --portal 192.168.166.70
...
* Konfigurieren Sie die HPE VME-Datenspeicher auf den ONTAP-LUNs und konfigurieren Sie dann die virtuelle Maschine. Weitere Informationen finden Sie in der HPE-Dokumentation des Anbieters.
