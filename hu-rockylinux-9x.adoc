...
summary: Installieren Sie Linux Host Utilities auf einem Rocky Linux 9.x Host, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Rocky Linux 9.x für FCP und iSCSI mit ONTAP -Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software bietet Verwaltungs- und Diagnosetools für Linux-Hosts, die mit ONTAP Speicher verbunden sind. Wenn Sie Linux Host Utilities auf einem Rocky Linux 9.x-Host installieren, können Sie die Host Utilities verwenden, um FCP- und iSCSI-Protokolloperationen mit ONTAP -LUNs zu verwalten.
...
Mit Rocky Linux 9.x können Sie Multipathing zur Verwaltung von ONTAP LUNs nutzen.
...
Rocky Linux 9.x mit ONTAP -Speicher weist folgende bekannte Probleme auf.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | Bei der NetApp Linux Host Utilities CLI sind zusätzliche Abhängigkeiten von Bibliothekspaketen erforderlich, um die Erkennung von Emulex HBA-Adaptern zu unterstützen | In Rocky Linux 9.1 ist die NetApp Linux SAN Host Utilities CLI  `sanlun fcp show adapter -v` schlägt fehl, weil die Bibliothekspaketabhängigkeiten zur Unterstützung der HBA-Erkennung nicht gefunden werden können. 
|===
--
====
...
* Erfahren Sie mehr über Rocky Linux Virtualisierung (KVM).
+
Rocky Linux kann als KVM-Host dienen. Dies ermöglicht es Ihnen, mehrere virtuelle Maschinen auf einem einzigen physischen Server mithilfe der Linux Kernel-based Virtual Machine (KVM)-Technologie auszuführen. Der KVM-Host benötigt keine expliziten Hostkonfigurationseinstellungen für ONTAP LUNs.
