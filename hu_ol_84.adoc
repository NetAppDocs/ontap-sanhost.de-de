...
summary: Installieren Sie Linux Host Utilities auf einem Oracle Linux 8.4-Host, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie Oracle Linux 8.4 für FCP und iSCSI mit ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Sie können Multipathing mit Oracle Linux 8.4 verwenden, um ONTAP-LUNs zu verwalten.


NOTE: Sie können die link:hu_rhel_84.html#rhel-rhck["empfohlene Einstellungen für Red Hat Enterprise Linux (RHEL) 8.4"] um den Red Hat Compatible Kernel für Oracle Linux 8.4 zu konfigurieren.
