...
---
= Konfigurieren Sie Rocky Linux 9.5 für NVMe-oF mit ONTAP Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Rocky Linux 9.5 konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP Unterstützung und Funktionen"]Die

NVMe-oF mit Rocky Linux 9.5 hat keine bekannten Einschränkungen.
...
.Schritte
. Installieren Sie Rocky Linux 9.5 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen Rocky Linux 9.5-Kernel ausführen:
+
[source, cli]
----
uname -r
