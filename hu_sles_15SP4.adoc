...
---
= Einsatz von SUSE Linux Enterprise Server 15 SP4 mit NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Ersetzen Sie den `<DevId>` Mit dem `WWID` Zeichenfolge des Geräts, das Sie ausschließen möchten. Verwenden Sie den folgenden Befehl, um das zu bestimmen `WWID`:
...
----
. Fügen Sie diese hinzu `WWID` Zur schwarzen Liste steht in der `/etc/multipath.conf`:
+
[listing]
----
blacklist {
...
Die nachfolgende Tabelle enthält die wichtigen Multipathd-Parameter für ONTAP LUNs und die erforderlichen Werte. Wenn ein Host mit LUNs anderer Anbieter verbunden ist und einer dieser Parameter außer Kraft gesetzt wird, müssen sie später in korrigiert werden `multipath.conf` Die speziell für ONTAP LUNs gelten. Wenn dies nicht erfolgt, funktionieren die ONTAP LUNs möglicherweise nicht wie erwartet. Diese Standardeinstellungen sollten nur in Absprache mit NetApp und/oder dem Anbieter des Betriebssystems außer Kraft gesetzt werden und nur dann, wenn die Auswirkungen vollständig verstanden wurden.
...
Für SUSE Linux Enterprise Server 15 SP4 sind keine Probleme bekannt.
