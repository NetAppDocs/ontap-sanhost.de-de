...
summary: Installieren Sie die Linux Host Utilities auf einem SUSE Linux Enterprise Server 15 SP2, um FCP- und iSCSI-Protokollvorgänge mit ONTAP-LUNs zu verwalten. 
---
= Konfigurieren Sie SUSE Linux Enterprise Server 15 SP2 für FCP und iSCSI mit ONTAP Storage
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software enthält Management- und Diagnose-Tools für Linux-Hosts, die mit dem ONTAP-Storage verbunden sind. Wenn Sie die Linux Host Utilities auf einem SUSE Linux Enterprise Server 15 SP2-Host installieren, können Sie mithilfe der Host Utilities FCP- und iSCSI-Protokollvorgänge mit ONTAP-LUNs managen.
...
Sie können Multipathing mit SUSE Linux Enterprise Server 15 SP2 zur Verwaltung von ONTAP-LUNs verwenden.
...
Die Version SUSE Linux Enterprise Server 15 SP2 mit ONTAP Storage weist folgende bekannte Probleme auf:
