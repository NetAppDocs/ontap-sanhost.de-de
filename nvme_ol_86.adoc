...
* Mit Oracle Linux 8.6 ist das NetApp Plug-in der nativen Version integriert `nvme-cli` Das Paket kann ONTAP-Details sowie ONTAP-Namespaces anzeigen.
...
.Schritte
. Stellen Sie sicher, dass Sie den unterstützten Adaptertreiber und die Firmware-Versionen ausführen. Der native Inbox qla2xxx Treiber im OL 8.6 GA Kernel enthält die neuesten Upstream-Fixes, die für die ONTAP Unterstützung unerlässlich sind.
+
[listing]
----
...
round-robin
----
. Vergewissern Sie sich, dass die Namespaces erstellt und auf dem Host richtig erkannt wurden.
+
[listing]
----
# nvme list
