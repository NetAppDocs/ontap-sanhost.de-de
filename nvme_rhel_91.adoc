...
. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen RHEL 9.1-Kernel ausführen. Siehe link:https://mysupport.netapp.com/matrix/["NetApp Interoperabilitäts-Matrix-Tool"^] Erhalten Sie auf der aktuellen Liste der unterstützten Versionen.
+
Beispiel:
...
--
Im nativen Posteingang `qla2xxx` Der im RHEL 9.1-Kernel enthaltene Treiber verfügt über die neuesten Fehlerbehebungen, die für die ONTAP-Unterstützung unerlässlich sind.
