...
summary: Konfiguration eines NVMe-oF-Hosts für RHEL 8.x mit ONTAP 
---
= RHEL 8.x für NVMe-oF mit ONTAP -Speicher konfigurieren
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für RHEL 8.x konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rhel-supported-features.html["RHEL ONTAP Unterstützung und -Funktionen"]Die

NVMe-oF unter RHEL 8.x weist folgende bekannte Einschränkungen auf:

* SAN-Booten mit dem NVMe-oF-Protokoll wird derzeit nicht unterstützt.
* In-Kernel NVMe Multipath ist auf NVMe-oF-Hosts in RHEL 8.x standardmäßig deaktiviert; Sie müssen es manuell aktivieren.
...
.Schritte
. Installieren Sie RHEL 8.x auf dem Server. Nach Abschluss der Installation überprüfen Sie, ob Sie den erforderlichen RHEL 8.x-Kernel ausführen:
+
[source, cli]
----
uname -r
...
----
. Überprüfen Sie auf dem RHEL 8.x-Host die `hostnqn` Zeichenkette bei `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
[listing]
----
nqn.2014-08.org.nvmexpress:uuid:4c4cxxxx-0032-xxxx-8035-b8cxxxxc5132
----
...
----------- --------------- ----------------------------------------------------------
vs_25_2742   rhel_101_QLe2772    nqn.2014-08.org.nvmexpress:uuid:5463xxxx-160f-xxxx-89aa-98bxxxx440ca
----
====
+
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | RHEL 8.x NVMe-oF-Hosts erstellen doppelte persistente Discovery Controller (PDCs). | Auf NVMe-oF-Hosts können Sie mit dem Befehl "nvme discover -p" PDCs erstellen. Bei Verwendung dieses Befehls sollte pro Initiator-Ziel-Kombination nur ein PDC erstellt werden. Wenn Sie jedoch RHEL 8.x auf einem NVMe-oF-Host ausführen, wird jedes Mal, wenn "nvme discover -p" ausgeführt wird, ein doppelter PDC erstellt. Dies führt zu einer unnötigen Ressourcennutzung sowohl auf dem Host- als auch auf dem Zielsystem. 
|===