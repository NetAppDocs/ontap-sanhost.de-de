...
summary: Beschreibt die Verwendung von CentOS 8.5 mit ONTAP 
---
= Verwenden Sie CentOS 8.5 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP-SAN-Hosts verwenden, um CentOS 8.5 mit ONTAP als Ziel zu konfigurieren.
...
Für CentOS 8.5 die `/etc/multipath.conf` Die Datei muss vorhanden sein, Sie müssen jedoch keine spezifischen Änderungen an der Datei vornehmen. CentOS 8.5 wird mit allen Einstellungen zusammengestellt, die zum Erkennen und korrekten Managen von ONTAP LUNs erforderlich sind.
...
CentOS 8.5 OS wird kompiliert, um ONTAP-LUNs zu erkennen und automatisch alle Konfigurationsparameter sowohl für ASA als auch für nicht-ASA-Konfiguration korrekt festzulegen.
...
Es gibt keine bekannten Probleme für das CentOS 8.5 mit ONTAP-Version.
