...
summary: Installieren Sie die Linux Host Utilities auf einem SUSE Linux Enterprise Server 16, um FCP- und iSCSI-Protokolloperationen mit ONTAP LUNs zu verwalten. 
---
= Konfigurieren Sie SUSE Linux Enterprise Server 16 für FCP und iSCSI mit ONTAP Storage
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Die Linux Host Utilities-Software bietet Verwaltungs- und Diagnosewerkzeuge für Linux-Hosts, die mit ONTAP-Speicher verbunden sind. Wenn Sie die Linux Host Utilities auf einem SUSE Linux Enterprise Server 16-Host installieren, können Sie die Host Utilities verwenden, um FCP- und iSCSI-Protokollvorgänge mit ONTAP LUNs zu verwalten.
...
Sie können Multipathing mit SUSE Linux Enterprise Server 16 verwenden, um ONTAP LUNs zu verwalten.
