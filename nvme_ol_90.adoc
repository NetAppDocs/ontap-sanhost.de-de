...
[role="lead"]
NVMe over Fabrics (NVMe-of), einschließlich NVMe/FC- und anderer Übertragungen, wird mit Oracle Linux 9.0 mit Asymmetric Namespace Access (ANA) unterstützt. In NVMe-of Umgebungen entspricht ANA ALUA Multipathing in iSCSI- und FC-Umgebungen und wird mit in-Kernel NVMe Multipath implementiert.
...
.Schritte
. Installieren Sie Oracle Linux 9.0 GA auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen Oracle Linux 9.0 GA-Kernel ausführen.
+
[listing]
----
# uname -r
...
--
.Schritte
. Stellen Sie sicher, dass Sie den unterstützten Adaptertreiber und die Firmware-Versionen ausführen. Der native Inbox qla2xxx Treiber im OL 9.0 GA Kernel enthält die neuesten Upstream-Fixes, die für die ONTAP-Unterstützung notwendig sind:
+
[listing]
----
...
.Schritte
. Überprüfen Sie die folgenden NVMe/FC-Einstellungen auf dem Oracle Linux 9.0-Host:
+
[listing]
----
...
Wenn Sie Probleme mit Ihrer Konfiguration haben, kann die ausführliche Protokollierung wertvolle Informationen liefern, die für die Fehlerbehebung erforderlich sind.
