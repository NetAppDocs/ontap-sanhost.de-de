...
Mit Rocky Linux 10, dem  `nvmefc-boot-connections.service` Und  `nvmf-autoconnect.service` In NVMe/FC enthaltene Boot-Dienste  `nvme-cli` Paket werden beim Systemstart automatisch aktiviert. Überprüfen Sie nach dem Booten, ob die Dienste aktiviert sind.
