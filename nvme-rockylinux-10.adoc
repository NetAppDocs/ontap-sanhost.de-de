...
summary: So konfigurieren Sie einen NVMe-oF-Host für Rocky Linux 10.0 mit ONTAP Speicher 
---
= Konfigurieren Sie Rocky Linux 10.0 für NVMe-oF mit ONTAP Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Rocky Linux 10.0 konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP Unterstützung und Funktionen"]Die

*NVMe-oF mit Rocky Linux 10.0 hat die folgenden bekannten Einschränkungen:*
...
.Schritte
. Installieren Sie Rocky Linux 10.0 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen Rocky Linux 10.0-Kernel ausführen:
+
[source, cli]
----
uname -r
...
Ab ONTAP 9.12.1 wird die sichere In-Band-Authentifizierung über NVMe/TCP zwischen einem Rocky Linux 10.0-Host und einem ONTAP Controller unterstützt.
...
cat /etc/nvme/hostnqn
----
. Generieren Sie den dhchap-Schlüssel für den Rocky Linux 10.0-Host.
+
