...
---
= Konfigurieren Sie RHEL 8.5 für NVMe-oF mit ONTAP Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für RHEL 8.5 konfigurieren.  Weitere Informationen zu Support und Funktionen finden Sie unterlink:hu-nvme-index.html["NVME-oF-Übersicht"^] .

*NVMe-oF mit RHEL 8.5 weist die folgenden bekannten Einschränkungen auf:*

* SAN-Booten mit dem NVMe-oF-Protokoll wird derzeit nicht unterstützt.
* In-Kernel-NVMe-Multipath ist auf NVMe-oF-Hosts in RHEL 8.5 standardmäßig deaktiviert; Sie müssen es manuell aktivieren.
...
.Schritte
. Installieren Sie RHEL 8.5 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen RHEL 8.5-Kernel ausführen:
+
[source, cli]
----
uname -r
...
----
. Überprüfen Sie auf dem RHEL 8.5-Host die `hostnqn` Zeichenfolge bei `/etc/nvme/hostnqn` :
+
[source, cli]
----
