...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Red Hat Enterprise Linux (RHEL) 9.5 konfigurieren.  Weitere Informationen zu Support und Funktionen finden Sie unterlink:hu-nvme-index.html["NVME-oF-Übersicht"^] .

NVMe-oF mit RHEL 9.5 hat keine bekannten Einschränkungen.
...
.Schritte
. Installieren Sie RHEL 9.5 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen RHEL 9.5-Kernel ausführen:
+
[source, cli]
----
uname -r
