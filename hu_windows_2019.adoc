...
---
= Verwenden Sie Windows Server 2019 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP-SAN-Hosts verwenden, um Windows Server 2019 mit ONTAP für den Betrieb mit ONTAP-Speicher zu konfigurieren.
...
Sie müssen die Microsoft Multipath I/O (MPIO)-Software installieren und Multipathing aktivieren, wenn Ihr Windows-Host über mehr als einen Pfad zum Speichersystem verfügt. Wenn Sie MPIO-Unterstützung auswählen, aktiviert Windows Host Utilities die MPIO-Funktion, die in Windows Server 2019 enthalten ist. Wenn Sie die Windows Host Utilities nicht verwenden, müssen Sie Multipathing manuell aktivieren.
...
Es gibt keine bekannten Probleme für Windows Server 2019 mit ONTAP-Version.
