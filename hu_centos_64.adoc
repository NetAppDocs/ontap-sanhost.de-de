...
summary: Beschreibt die Verwendung von CentOS 6.4 mit ONTAP 
---
= Verwenden Sie CentOS 6.4 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Sie können die Konfigurationseinstellungen des ONTAP-SAN-Hosts verwenden, um CentOS 6.4 mit ONTAP als Ziel zu konfigurieren.
...
Für CentOS 6.4 die `/etc/multipath.conf` Die Datei muss vorhanden sein, Sie müssen jedoch keine spezifischen Änderungen an der Datei vornehmen. CentOS 6.4 wird mit allen Einstellungen zusammengestellt, die zum Erkennen und korrekten Managen von ONTAP LUNs erforderlich sind.
...
Das CentOS 6.4 Betriebssystem wird kompiliert, um ONTAP-LUNs zu erkennen und automatisch alle Konfigurationsparameter sowohl für ASA als auch für nicht-ASA-Konfiguration korrekt einzustellen.
...
Für CentOS (Red hat kompatibler Kernel) bekannte Probleme siehe link:hu_rhel_64.html#known-problems-and-limitations["Bekannte Probleme"] Für Red hat Enterprise Linux (RHEL) 6.4.
