...
== Aktivieren Sie die Speicherplatzzuweisung

Die Speicherplatzzuweisung wird für ESXi 8.x und höher unterstützt.

Wenn die Speicherplatzzuweisung aktiviert ist und einem Namespace der Speicherplatz ausgeht, kommuniziert ONTAP mit dem Host, dass kein freier Speicherplatz für Schreibvorgänge verfügbar ist. Der Namespace bleibt online und Lesevorgänge werden weiter verarbeitet. Die Schreibvorgänge werden wieder aufgenommen, sobald zusätzlicher freier Speicherplatz verfügbar wird. Die Platzzuweisung erlaubt dem Host auch Operationen durchzuführen `UNMAP` (manchmal auch genannt `TRIM`). Durch UNMAP-Vorgänge kann ein Host Datenblöcke identifizieren, die nicht mehr benötigt werden, da sie keine gültigen Daten mehr enthalten. Das Storage-System kann diese Datenblöcke dann Zuordnung aufheben, sodass der Speicherplatz an anderer Stelle verbraucht werden kann.

.Bevor Sie beginnen
link:https://docs.netapp.com/us-en/ontap/san-admin/enable-space-allocation.html["Aktivieren Sie die Speicherplatzzuweisung auf Ihrem ONTAP Storage-System"^]. Dann sollten Sie die folgenden Schritte auf Ihrem ESXi-Host durchführen.
