...
---
= Konfigurieren Sie Rocky Linux 8.5 für NVMe-oF mit ONTAP Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Erfahren Sie, wie Sie NVMe over Fabrics (NVMe-oF)-Hosts für Rocky Linux 8.5 konfigurieren. Weitere Informationen zu Support und Funktionen finden Sie unter link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP Unterstützung und Funktionen"]Die

*NVMe-oF mit Rocky Linux 8.5 hat die folgenden bekannten Einschränkungen:*

* SAN-Booten mit dem NVMe-oF-Protokoll wird derzeit nicht unterstützt.
* In-Kernel-NVMe-Multipath ist auf NVMe-oF-Hosts in Rocky Linux 8.5 standardmäßig deaktiviert; Sie müssen es manuell aktivieren.
...
.Schritte
. Installieren Sie Rocky Linux 8.5 auf dem Server.  Überprüfen Sie nach Abschluss der Installation, ob Sie den erforderlichen Rocky Linux 8.5-Kernel ausführen:
+
[source, cli]
----
uname -r
