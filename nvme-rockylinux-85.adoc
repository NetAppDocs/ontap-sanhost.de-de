...
summary: So konfigurieren Sie einen NVMe-oF-Host für Rocky Linux 8.5 mit ONTAP 
---
= Konfigurieren Sie Rocky Linux 8.5 mit NVMe-oF für ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.5-Hosts unterstützen die Protokolle NVMe/FC und NVMe/TCP mit Asymmetric Namespace Access (ANA). ANA entspricht dem Asymmetric Logical Unit Access (ALUA) Multipathing in iSCSI- und FCP-Umgebungen und wird mithilfe der In-Kernel-NVMe-Multipath-Funktion implementiert.
...
.Über diese Aufgabe
Sie können die folgende Unterstützung und Funktionen mit der NVMe-oF-Hostkonfiguration für Rocky Linux 8.5 verwenden. Sie sollten auch die bekannten Einschränkungen überprüfen, bevor Sie mit dem Konfigurationsprozess beginnen.
...
+
** In-Kernel-NVMe-Multipath ist für Rocky Linux 8.5 NVMe-oF-Hosts standardmäßig deaktiviert. Deshalb müssen Sie sie manuell aktivieren.
** Auf Rocky Linux 8.5-Hosts ist NVMe/TCP aufgrund offener Probleme eine Technologievorschaufunktion. Weitere Informationen finden Sie im https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.5_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.5 Versionshinweise"^] .
...
Verwenden Sie das folgende Verfahren, um die mindestens unterstützten Softwareversionen von Rocky Linux 8.5 zu validieren.

.Schritte
. Installieren Sie Rocky Linux 8.5 auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen Rocky Linux 8.5-Kernel verwenden:
+
[source, cli]
----
uname -r
...
Es gibt keine bekannten Probleme mit der NVMe-oF-Hostkonfiguration unter Rocky Linux 8.5 mit ONTAP-Version.
