...
---
= Konfigurieren Sie Rocky Linux 8.10 mit NVMe-oF für ONTAP-Speicher
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.10-Hosts unterstützen die Protokolle NVMe/FC und NVMe/TCP mit Asymmetric Namespace Access (ANA). ANA entspricht dem Asymmetric Logical Unit Access (ALUA) Multipathing in iSCSI- und FCP-Umgebungen und wird mithilfe der In-Kernel-NVMe-Multipath-Funktion implementiert.
...
.Über diese Aufgabe
Sie können die folgende Unterstützung und Funktionen mit der NVMe-oF-Hostkonfiguration für Rocky Linux 8.10 verwenden. Sie sollten auch die bekannten Einschränkungen überprüfen, bevor Sie mit dem Konfigurationsprozess beginnen.
...
+
** In-Kernel-NVMe-Multipath ist für Rocky Linux 8.10 NVMe-oF-Hosts standardmäßig deaktiviert. Deshalb müssen Sie sie manuell aktivieren.
** Auf Rocky Linux 8.10-Hosts ist NVMe/TCP aufgrund offener Probleme eine Technologievorschaufunktion. Weitere Informationen finden Sie im https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/8/html-single/8.10_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.10 Versionshinweise"^] .
...
Verwenden Sie das folgende Verfahren, um die mindestens unterstützten Softwareversionen von Rocky Linux 8.10 zu validieren.

.Schritte
. Installieren Sie Rocky Linux 8.10 auf dem Server. Überprüfen Sie nach Abschluss der Installation, ob Sie den angegebenen Rocky Linux 9.6-Kernel verwenden:
+
[source, cli]
----
uname -r
...
Die NVMe-oF-Hostkonfiguration für Rocky Linux 8.10 mit ONTAP weist das folgende bekannte Problem auf:
