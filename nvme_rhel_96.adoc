...
---
= NVMe-of Hostkonfiguration für RHEL 9.6 mit ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
----
. Überprüfen Sie auf dem RHEL 9.6-Host den hostnqn-String unter `/etc/nvme/hostnqn`:
+
[source, cli]
----
...
Mit RHEL 9.6 wird die  `nvmefc-boot-connections.service` Und  `nvmf-autoconnect.service` In NVMe/FC enthaltene Boot-Dienste  `nvme-cli` Paket wird beim Systemstart automatisch gestartet. Überprüfen Sie nach Abschluss des Systemstarts, ob die Startdienste aktiviert sind.
